...
cycles will be $max(C(div), C(sub)) + C(mul) = C(div) + C(mul)$; since $C(div)$ is
usually much greater than $C(sub)$ \cite{x86,aarch64}.

A partial redundancy elimination pass could now remove the entire if-block
because final operation is an addition (asociative under fast math), and the end
result is:
...
The code shown in Section~\ref{subsec:example} is a reduced test that appears in
a hot loop of a proprietary benchmark.  When the expressions are hoisted from
