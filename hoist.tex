...
\begin{abstract}
  ...
  saves code size, a) It exposes redundancies in some cases, b) It exposes more
  instruction level parallelism in the basic block a computation is hoisted, and
  c) It enables other passes like LICM to remove more redundancies.
...
\item to improve function inlining heuristics: functions become cheaper to
  inline when thier code size decreases because inliner heuristics in \LLVM{}
...
We ran \LLVMTestSuite{} (trunk:d87471f8) with the patch (trunk:86940146) and the
