...
\begin{itemize}
\item hoisting of identical computations
\item partition algorithm
\item code hoisting can remove fully redundant computations
\item code hoisting can actually improve performance in some cases
...
the instructions before finding candidates (optimistic).
...
non-leaf nodes (bottom-up) because instructions are hoisted upwards. The
pessimistic algorithm is fast, results in fewer spills but hoists very less
...
It also depends on the generality of the GVN algorithm, however, that analysis
...
computations and partially hoist a set of computations to their common
dominator. The partition algorithm is described as follows:

\subsubsection{Partition the list of hoisting candidates to maximize hoisting}
\label{subsec:partition}
The set of candidates are sorted in the order of their DFS-in numbers. So that
candidates close to each other in the list are also closer (??) in the CFG.
