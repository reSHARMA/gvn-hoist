...
GCC implementation: Hoists top down in a predecessor where the value is in
ANTIC\_IN. This approach may have the problem of hoisting the expression too far
and to mititage that it only hoists expressions when at least one successor
computes it. Hence the algorithm will hoist a limited set of computations.  One
advantage of GCC implementation is that it works in sync with the GVN-PRE such
that when new hoisting opportunities are created by GVN-PRE, code-hoisting will
hoist them.
