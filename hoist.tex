...
the hoisting point, we find if there are aliasing stores, we find if any of the
basic blocks are landing pads or targets of indirect branches. In those cases we
...
from the current position through the hoisting point and, none of the basic
blocks along the path have their address taken.

\subsubsection{Hoisting calls}
We have to check that the loads/stores in the call does not create hazard with
any loads/stores (in the caller) in the path from callsite through the hoisting
...
after a call (may be as a different pass).
