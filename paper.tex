...
Code size remained almost the same as a result of code-motion as shown in Table~\cite{tab:code-size}.
The code size listed here is the text size (from linux size command) of the final executables for each benchmark.
Some bencmarks like 473.astar increased in code size by almost 2\%. This is because once the code-size
of a function decreases it becomes cheaper to inline. As we can see in Table-\cite{tab:stats}, four
more functions got inlined in 473.astar. Except for 447.dealII, 450.soplex, 482.sphinx3 and 483.xalancbmk
all other benchmarks got more functions inlined.

\begin{table}[h]
  \begin{center}
    \begin{tabular}{|l|c|c|c|}
      \hline
  Benchmark       &   Baseline	        &  GCM & \%Incrase     \\\hline
