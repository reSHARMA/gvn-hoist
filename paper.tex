...
\gcm{} increases instruction level parallelism thereby enabling more out of
...
instcombine might actually remove those loads and the number of hoisted loads may change should
the \GCM{} pass be scheduled later. The number of instructions sunk is very less
because \LLVM{} has a code sinking and hoisting transform inside SimplifyCFG pass which runs before
\GCM{} and sinks and hoists some instructions with some rather severe limitations. Another reason is that instructions for which
