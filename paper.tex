...
\SPEC{} bencmarks suite when compiled for \xlinux{}. This is an optimistic
...
this pass (before mem2reg pass which scalarizes some memory references) in the \LLVM{} pass pipeline. Passes like mem2reg and
...
is the total instruction count as outputted from valgrind (--tool=cachegrind) \cite{valgrind}.
...
the codesize of the final binaries. Since the pass runs early, it affects many
...
in inlining decisions (Table~\ref{tab:code-size}), when \gcm{}
