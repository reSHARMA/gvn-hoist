...
\section{A fastt Code motion of congruent computations}
...
frontiers and the post-dominance frontiers of the CFG. These points, being
structural properties of a graph, do not change during code-motion.

A post-dominance frontier is the basic block where anticipability (ANTIC) of an
...
of a basic block. We only do this for values with multiple occurrences in the
function as they are the potentially hoistable candidates. This approach allows
us to hoist instructions to a basic block with more than two successors, as well
as deal with infinite loops in a trivial way.
...
\begin{figure}
  ...
  \vspace*{-1.0cm}
\caption{I1, I2 are instructions in B, and C respectively, CHIArg1 = \{V, B,
  I1\} and CHIArg2 = \{V, C, I2\}, V is the value number for both I1, and I2}
\label{fig:chi-intro}
\end{figure}
...
Essentially, CHIArg1 = \{V, B, I1\} and CHIArg2 = \{V, C, I2\}. For sinking we
use the similar approach as \cite{ssapre} to track values flowing into a basic
block.
...
\begin{itemize}
\item Sort VNs according to their rank, and start with lowest ranked VN
\item Take a VN and for each instruction with same VN
\item  - Find the dominance frontier in the inverse graph (PDF)
\item  - Insert the chi-node at PDF
\item Remove the chi-nodes with missing entries
\item Remove values from CHI-nodes which do not truly flow out, e.g.,
\item  modified along the path.
\item Collect the remaining values that are still anticipable
\end{itemize}
...
(greedy approach.) To start with, the first two candidates are tried for
