...
%% Title information
\title[SSA GCM]{A fast algorithm for global code motion of congruent instructions}         %% [Short Title] is optional;
...
\begin{abstract}
We present a fast \gcm{} (\GCM{}) compiler optimization which schedules
congruent instructions across the program. We introduce data structures to help
exploit flow of global values in sparse representation.  Not only \GCM{}
improves performance it saves code size as well, it exposes more instruction
level parallelism in the basic-block to which instructions are moved, and it
enables other compiler optimizations like loop invariant motion to remove
redundancies. The cost model to drive the code motion is based on liveness
analysis on \SSA{} representation such that the (virtual) register pressure does
not increase resulting in 2\% fewer spills on the \SPEC{} benchmark
suite. \GCM{} is an optimistic algorithm in the sense that it considers all
congruent instructions in a function as potential candidates. We also formalize
why register pressure reduces as a result of \gcm{}, and how \gcm{} increases
instruction level parallelism thereby enabling more \ooo{} execution on modern
architectures.

We have implemented the pass in \LLVM{}. The experimental results show
performance gains of 8\% in 462.libquantum and 5.3\% in 471.omnetpp from the
\SPEC{} benchmark suite. It also reduces compilation time by reducing number of
instructions to be processed by later compilation stages. \GCM{} enables more
...
are marked as barriers. A barrier will prevent any instruction occurring
lexically below it from getting hoisted. On the other hand, a sink barrier will
prevent any instruction above it from getting sunk. If there are no barriers in
a basic block, any instruction satisfying other legality checks can be sunk.

\begin{lstlisting}[caption={Compute hoist and sink barriers},captionpos=b]
...
\begin{lstlisting}[caption={Global Code Motion},captionpos=b]
void doGCM()
  Analyses available:
...
\begin{lstlisting}[caption={Hoist instructions},captionpos=b]
...
\begin{lstlisting}[caption={Sink instructions},captionpos=b]
int doCodeSinking()
  // VNs = map (VN -> list of I with value VN)
  Sort VNs in ascending order
...
% llvm trunk vs. llvm trunk with code-motion
                &\% performance uplift & size reduction (Bytes) \\
Benchmarks      & at -Ofast (high better)& at -Ofast (high better)\\\hline
400.perlbench	&  1.95  & 984   \\\hline
401.bzip2	& -0.84  & 16    \\\hline
403.gcc	        & -2.30  & 2128  \\\hline
429.mcf	        & -4.36  & 16    \\\hline
433.milc	& -1.86  & 48    \\\hline
444.namd	& -2.40  & 544   \\\hline
445.gobmk	& -0.52  & 56    \\\hline
447.dealII	&  0.39  & -2560 \\\hline
450.soplex	& -0.24  & -32   \\\hline
453.povray	& -0.05  & 848   \\\hline
456.hmmer	&  0.93  & 296   \\\hline
458.sjeng	& -0.58  & 232   \\\hline
462.libquantum	&  8.17  & -16   \\\hline
470.lbm	        &  2.57  & 0     \\\hline
471.omnetpp	&  5.30  & 152   \\\hline
473.astar	& -0.17  & 48    \\\hline
482.sphinx3	& -0.11  & 16    \\\hline
483.xalancbmk	& -2.98  & 392   \\\hline
    \end{tabular}}
  \end{center}
...
Overall, we can see some good improvements in 462.libquantum, 471omnetpp, etc.
and some regressions in 429.mcf, 483.xalancbmk, etc. The \gcm{} pass was run once
in the pass pipeline (at -Ofast), after local common subexpression elimination
(EarlyCSE) pass. Because EarlyCSE removes local redundancies, \GCM{} would have
to analyze less redundant instructions. We believe some performance regressions
can be mitigated by running the \GCM{} few more times.
...
\begin{table}[h!]
 ...
 483.xalancbmk	  & 14.6& -3.6 &  -1.0&  -1.2 \\\hline
    \end{tabular}}
  \end{center}
  \caption{Change in the static compile time metrics w.r.t. \GCM{} on \SPEC{} at
    -Ofast. The \% columns show percentage increase in metric w.r.t. baseline,
    i.e., without \GCM{}. \GCM{} improves LICM and inlining in general.}
  \label{tab:stats}
\end{table}

Table~\ref{tab:stats}, which shows how \gcm{} impacted important compiler
optimizations like inlining and loop invariant code motion (LICM). LICM removes
loop invariant code out of the loop. It may hoist the code to the loop's
preheader or sink the code to loop's post-dominator
...
deleted as in 401.bzip2, 445.gobmk.
...
Grand Total	  & 43349	& 42497	& 0.98 \\\hline
    \end{tabular}}
  \end{center}
  \caption{Number of spills with and without\GCM{} on SPEC2006 at
    -Ofast. Lower is better in GCM/base. Spills reduced by 2\%.}
  \label{tab:spills}
\end{table}
...
\begin{table}[h]
  ...
  \caption{Change in total number of instructions executed during the
    compilation with and without \GCM{} on \SPEC{} at -Ofast. The
...
-{}-tool=cachegrind' \cite{valgrind}. We used valgrind because it was a simple
and reliable way to show the impact of our optimization.
...
could improve performance. It may be useful to integrate \GCM{} with \LLVM{'}s
